...
yargs(hideBin(process.argv))
    ...
    .command(
        ...
        (argv) => {
            ...
            if (onlyClosed && !onlyOpened)
                issues$ = gitlabService.getIssuesWithState(
                    projectName,
                    IssueState.CLOSED,
                    quantity
                );
            ...
            else issues$ = gitlabService.getAllIssues(projectName, quantity);
            issues$.subscribe({
                next: (issues) => {
                    issues.forEach((i) =>
    ...
    .command(
        ...
        (args) => {
            ...
            const dumpSimple = config.dumpDetails();
            const gitlabService = config.createService();
            const issues$ = gitlabService.getMilestoneIssues(
                projectName,
                milestoneName
            );
            issues$.subscribe({
    ...
    .command(
        ...
        (args) => {
            ...
            const dumpSimple = config.dumpDetails();
            const issues$ = gitlabService.getReleaseIssues(
                projectName,
                releaseName
            );
            issues$.subscribe({
    ...
    .command(
        ...
        (args) => {
            ...
            const quantity = config.getQuantity();
            const issues$ = gitlabService.getIssuesWithLabel(
                projectName,
                labelName,
                quantity
            );
    ...
    .command(
        ...
        (args) => {
            ...
            const milestones$ = gitlabService.getMilestones(
                projectId,
                onlyActive,
                quantity
            );
            ...
            issues$.subscribe({
                next: (is) =>
                    is.forEach((i) => {
                        ...
                        const mod$ = gitlabService.swapIssueLabel(
                            projectName,
                            i,
                            fromLabel,
                            toLabel
            ...
            releases$.subscribe({
                next: (releases) => {
                    releases.forEach((r) => {
                        ...
                        milestones.forEach((m) =>
                            config.logger.printItem(m.toString(), 2)
                        );
                    });
                },
                error: (err) => {
    ...
    .command(
        ...
        (args) => {
            ...
            const release$ = gitlabService.getReleaseByNames(
                projectName,
                relasesName
            );
            release$.subscribe({
                next: (release) => {
                    ...
                    links.forEach((l) =>
                        config.logger.printItem(l.toString(), 2)
                    );
                },
                error: (err) => config.logger.exit(err)
            });
    ...
    .command(
        ...
        (args) => {
            ...
            const projectName = config.getPna();
            const quantity = config.getQuantity();
            const pipelines$ = gitlabService.getPipelines(
                projectName,
                quantity
            );
            pipelines$.subscribe({
    ...
    .command(
        ...
        (args) => {
            ...
            tags$.subscribe({
                next: (tags) =>
                    tags.forEach((t) => config.logger.printItem(t.toString())),
                error: (err) => config.logger.exit(err)
            });
        }
    )
    ...
    .showHelpOnFail(true)
    .epilogue(
        `For more information, check out the documentation at https://github.com/andreclinio/gitlab-cli`
    ).argv;
