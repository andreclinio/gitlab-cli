...
export class Issue extends Holder<JsonIssue> {
    ...
    toString(detailed: boolean): string {
        ...
        if (detailed) {
            const assTxt = this.unassigned
                ? `[${Logger.toYellow('assigned to nobody')}]`
                : this.assignees.map((as) => as.toString()).join(',');
            txt += `\n[Assigned to: ${assTxt}] `;
            txt += this.milestone
                ? `[Milestone: ${this.milestone.title} (${this.milestone.stateText})]`
                : `[${Logger.toYellow('No milestone')}]`;
    ...
    get timeText(): string {
        if (this.closed)
            return Logger.toGreen(`closed at ${this.dayOf(this.closed_at)}`);
        const daysToEndTxt =
            Math.abs(this.daysToEnd) > 30 ? '>30' : this.daysToEnd.toFixed(0);
